...
std::vector<MethodInfo> ConceptFinder::extract_methods(clang::ConceptDecl* decl) {
...
TypeErasureASTConsumer::TypeErasureASTConsumer(
...
void TypeErasureASTConsumer::HandleTranslationUnit(clang::ASTContext& context) {
    ...
    for (const auto& concept_info: visitor_.getConcepts()) {
        ...
        ofs << generated_code;
        llvm::outs() << "Generated type erasure for concept '" << concept_info.name << "' to "
                     << out_file << "\n";
    }
}
...
TypeErasureAction::CreateASTConsumer(clang::CompilerInstance& ci, llvm::StringRef file) {
    return std::make_unique<TypeErasureASTConsumer>(&ci.getASTContext(), output_dir_);
}
...
{4}
        ...
        concept_info.name,
        loc_str,
        generate_interface(concept_info),
        generate_adapter(concept_info),
        generate_wrapper(concept_info)
    );
}
