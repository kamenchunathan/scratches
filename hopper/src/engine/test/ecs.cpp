...
struct Velocity {
  ...
  float magnitude() const { return std::sqrt(dx * dx + dy * dy); }
};
...
struct Health {
  ...
  float health_ratio() const { return static_cast<float>(current) / max; }
};
...
void spawn_grid_entities(engine::ecs::World &world, float spacing = 2.0f,
                         int grid_size = 10) {
  ...
  for (int x = 0; x < grid_size; ++x) {
    for (int y = 0; y < grid_size; ++y) {
      ...
      if (x == 0 && y == 0) {
        // Player at origin
        world.create_entity(Position{pos_x, pos_y}, Velocity{0.0f, 0.0f},
                            Health{200}, Player{"Hero", 5});
      } else if ((x + y) % 7 == 0) {
        // Some enemies (every 7th position)
        world.create_entity(Position{pos_x, pos_y},
                            Velocity{vel_dist(gen), vel_dist(gen)},
                            Health{health_dist(gen)}, Enemy{aggro_dist(gen)});
      } else if ((x + y) % 5 == 0) {
...
void create_some_dead_entities(engine::ecs::World &world) {
  ...
  world.create_entity(Position{-3.0f, -3.0f}, Velocity{0.0f, 0.0f}, Health{0},
                      Dead{});
}
...
int main() {
  ...
  auto fast_movers =
      moving_entities | std::views::filter([](const auto &tuple) {
  ...
  auto first_quadrant_positions = moving_entities |
  ...
  auto living_entities =
      engine::ecs::Query<Position, Health>(&world).without<Dead>();

  auto healthy_entities =
      living_entities | std::views::filter([](const auto &tuple) {
  ...
  auto dangerous_enemies =
      enemy_query | std::views::filter([](const auto &tuple) {
  ...
  auto valuable_collectibles =
      collectibles | std::views::filter([](const auto &tuple) {
  ...
  auto complex_pipeline =
      engine::ecs::Query<Position, Velocity, Health>(&world).without<Dead>() |
      std::views::filter([](const auto &tuple) {
      ...
      std::views::transform([](const auto &tuple) {
        ...
        return EntityInfo{entity, vel.magnitude(), health.health_ratio(),
  ...
  for (const auto &info : complex_pipeline) {
    printf("  Entity %u: speed=%.2f, health=%.0f%%, distance=%.1f\n",
           info.id, info.speed, (info.health_pct * 100), info.distance_from_origin);
  }
  ...
  auto health_values =
      all_health_entities | std::views::transform([](const auto &tuple) {
        auto [entity, health] = tuple;
        return health.current;
      });

  auto total_health = std::ranges::fold_left(health_values, 0, std::plus{});
  auto max_health = std::ranges::max(health_values);
  auto min_health = std::ranges::min(health_values);
  auto entity_count = std::ranges::distance(health_values);

  printf("\n=== Health Statistics ===\n");
  printf("Living entities: %ld\n", (long)entity_count);
  printf("Total health: %d\n", total_health);
  printf("Average health: %ld\n", (long)(entity_count > 0 ? total_health / entity_count : 0));
  printf("Health range: %d - %d\n", min_health, max_health);
