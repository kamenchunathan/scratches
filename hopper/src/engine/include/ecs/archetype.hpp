...
public:
  ...
  void reserve(std::size_t capacity) override { components_.reserve(capacity); }
...
public:
  ...
  template <typename T> const T &get_component(std::uint32_t index) const {
  ...
  std::uint32_t add_entity(Entity entity, Components &&...components) {
    ...
    std::uint32_t index =
        static_cast<std::uint32_t>(entity_indices_.size() - 1);
  ...
  bool matches_query(const ComponentMask &required,
                     const ComponentMask &disallowed) const {
    return (signature_.mask & required) == required &&
           (signature_.mask & disallowed) == 0;
  }
