...
class SetBuilder {
public:
    SetBuilder(Schedule& schedule, uint32_t set_id): schedule_(schedule), set_id_(set_id) {}
    ...
    void build() {
        ...
        if (sorted_sets.size() != in_degree.size()) {
            throw std::runtime_error(
                "Schedule Build Error: Circular dependency detected in system sets."
            );
        }
